...
K load(S s) //TODO: working dir is stable ... store then reset after reading scripts
{
  ...
  if(fCmplt==1) { kerr("open-in-next-line"); oerr(); O("%s\n",Line); }
  kerr("undescribed"); fer=fCmplt=fLoad=0; prompt(0);
  R _n();
}
...
Z K backslash_d(S s,I n,K*dict) {
  ...
  if(n==4 && s[3]==*"^") {
    ...
    if(strlen(__d)==2) {__d=""; R _n();}
    if(strlen(__d)>3) {
      I c=0,i=0;
      for(i=0;i<strlen(__d);i++) if(__d[i]==*".")c=i;
      strcpy(z,__d); z[c]=*"\0"; __d=(S)sp(z);
      R _n();
    }
  }
  ...
  if(isalpha(s[3])) { 
    denameD(dict,s+3,1);
    strcpy(z,__d); strcat(z,"."); strcat(z,s+3); __d=(S)sp(z);
    R _n();
  }
