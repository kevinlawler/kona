...
{ if(!*q)*q=newPDA();
  PDA p=*q; P(!p,-1)  C t; I r=formed_group('\0')+1;//row-length
  for(;p->i < n;p->i++)
...
{ if(m[i] || '`'!=s[i]) R 0;//#spaces marked
  I adot=0,j=0,k;
  while(i+2+j < n && -MARK_QUOTE==m[i+1] && MARK_QUOTE==m[i+2+j]) j++;   //case `"ss-sss"
  if(j) R j+2;
  //case plain syms
...
{ I c=0;
  ...
  if(i<n-1&&s[i]=='_'&&isalpha(s[i+1])&&(i==n-2||m[i+2]||!isalnumdot_(s[i+2]))) R 2;   //_A case
  while(i+c<n && isalnumdot_(s[i+c]))   // (\.?S\.?)+ case
...
{ if(s[0]=='\t' || s[0]=='\014') R 5;
  ...
  if(n>2)
  { for(i=2;i<n;++i)
    { if(s[i]=='\\' && s[i-1]==':' && (s[i-2]!='/' && s[i-2]!='\\') && s[i+1]!='n') R 80;
      if(s[i]=='/' &&
...
{ //IN string, string length, pos in string, markings;
  ...
  { CS(MARK_CONDITIONAL, z=offsetColon)   //dummy value
    CS(MARK_PAREN,    fbr=1; fdc=1;
                      z=wd_(s+k+1,r-2,dict,func); if(!z)R (L)PE;) //oom. currently z->t==7 z->n==0.
                      //Execution will know this is paren (for rev order) because of its depth
    CS(MARK_BRACKET,  fbr=1;
                      if(!*d || bk(p[-1]))
    ...
    CS(MARK_BRACE,    //Functions & subfunctions validated at parse time
                      ...
                      kV(z)[CODE]=g; ((K)z)->n=3; memcpy(kC(g),s+k+1,r-2); kV(z)[CONTeXT]=func?kV(func)[CONTeXT]:d_;
                      K* zdict=(K*)kV(z)+PARAMS; K* ydict=(K*)kV(z)+LOCALS; K j;
