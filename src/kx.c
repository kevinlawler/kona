...
{ K u=b,c=0; I flag=0,useN=0,n=0,useB=0; fsf=0;
  if(a){ if(1==a->t){useN=1; n=*kI(a); }
...
{ K tc=0;
  ...
  { K v=Kv(),kb=newK(-4,2); M(v,kb)  kK(kb)[0]=q; kK(kb)[1]=0; kV(v)[CODE]=kb;
    z=vf_ex(&v,g); //Punt and let another call to vf_ex handle projecting. Probably could build the projected-verb here instead.
    cd(v);
    GC; } //old comment: Projection? '(1+)' -> 1+  Build 7-verb? (Refactor with 'c' from []+: ex and maybe another place?)
  ...
  { CS(1, //Executing a derived verb such as 1+2* or (+/)
    ...
    CS(3, //Executing a {} character function such as {1+1}, {x+y+z-1}, or {[a;b] a+b}
          ...
          else if((t=(UI)kS(kK(fw)[CODE])[0]>DT_SIZE || (UI)kS(kK(fw)[CODE])[1]>DT_SIZE) )
          { if(t) cd(kV(f)[CACHE_WD]);
            K fc = kclone(f); //clone the function to pass for _f
            cd(kV(fc)[CONJ]); kV(fc)[CONJ]=0;
            kV(fc)[DEPTH]++;
...
{ V q=*p; K x; I n=0;   //assert 0!=k->n    assert k==b->n (otherwise, projection/VE, which shouldn't reach here)
  ...
  { DO(k->n-1, x=kK(k)[i+1];
               ...
               if(!atomI(x))
               { if(n&&n!=x->n)R LE;
                 else n=x->n ) }   //return x_0 if any empty list x_{i>0}
  ...
  { DO(k->n, x=kK(k)[i];
             ...
             if(!atomI(x))
             { if(n&&n!=x->n) R LE;
               else n=x->n) } //return () on any empty list
...
{ K t0,t2,t3,e,u; I i=0; ft3=0;
  ...
  { K z=ex_(*v,1);  // n case
    ...
    { if(prnt && kV(z)[PARAMS] && kV(prnt)[CACHE_TREE] && !kV(z)[CACHE_TREE] && !kK(z)[LOCALS]->n)
      ...
      if(prnt && kV(prnt)[CODE]  &&  kK(prnt)[CODE]->t==-3  &&  kC(kK(prnt)[CODE])[0]=='{'  &&
  ...
  { if(adverbClass(v[1])) R SYE;//Could potentially handle instead of erroring
    ...
    { K x = d; if(rc(x)){ d=kclone(x); cd(x); } }
    else if((*w)->t!=6){ K x = *w; if(rc(x)>1){ *w=kclone(x); cd(x); } }
    K h=dot_tetradic_2(w,b,c,d); cd(c); cd(d); M(b,h)  K j=of(h,b); cd(b); R j; }
