...
{ if(offsetColon==w[0] && (UI)w[1]>DT_SIZE && (UI)w[2]>DT_SIZE && fwh==0) { fer=1; if(f)*i=n; else *i=-1; K tmp=*(K*)*(w+1); R ci(tmp); }
...
{ K t0,t2,t3,e,u;  I i=0;
  ...
  if(!v[1] && !k) { K z=ex_(*v,1);  // n case
    ...
    { if(prnt && kV(z)[PARAMS] && kV(prnt)[CACHE_TREE] && !kV(z)[CACHE_TREE] && !kK(z)[LOCALS]->n)
      ...
      { if(encf) cd(encf);
        encf=ci(prnt); }
      if(encp!=2 || !prnt) { if(prnt) { if(grnt) cd(grnt);
                                        grnt=prnt; }
                             prnt=ci(z); }
      else {cd(z); R prnt;} }
    R z; }
  ...
  if(bk(v[1]))
  { K z= ex_(*v,1); if(fer==2 && !fCheck)R (K)0;
    if(prnt && z && z->t==7)
  ...
  { if(adverbClass(v[1]))R SYE;//Could potentially handle instead of erroring
    ...
    { U(b=ex_(kV(a)+CONJ,((L)*kW(b)==1 || (L)*(kW(b)+1)==1)?1:2))
      ...
      { if(1e6<(UI)w)
        { K r=*(K*)w;
          if(r->t==5)
          { p=enumerate(r); cd(b); b=enlist(p); cd(p); } } } }
    if(!b) U(b=newK(0,0))
