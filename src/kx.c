...
Z K each2(K a, V *p, K b)
{
  ...
  else
  {
    ...
    if(0 >bt) DO(bn, g=newK(ABS(bt),1); M(g,z) memcpy(g->k,((V)b->k)+i*bp(bt),bp(bt));
      if(*p==(V)offsetEach && (*(p-1)==(V)offsetOver || *(p-1)==(V)offsetScan) && *(p-2)<(V)DT_SIZE)d=dv_ex(a,p-1,g);
      else d=dv_ex(0,p-1,g);
      cd(g); M(d,z) kK(z)[i]=d)
    if(0==bt) DO(bn,
      if(*p==(V)offsetEach && (*(p-1)==(V)offsetOver || *(p-1)==(V)offsetScan) && *(p-2)<(V)DT_SIZE)d=dv_ex(a,p-1,kK(b)[i]);
    ...
    if(t3>(K)DT_SIZE && t3->t==7 && t3->n==3) {
      if(prnt && kV(prnt)[CACHE_TREE] && kV(prnt)[CACHE_WD] && !kK(t3)[LOCALS]->n) {
        if(kK(prnt)[CACHE_TREE]->n) {
          ...
          K j2=join(j0,j1); cd(kK(t3)[CACHE_TREE]); kV(t3)[CACHE_TREE]=dot_monadic(j2); cd(j0); cd(j1); cd(j2); fsf=1; }
