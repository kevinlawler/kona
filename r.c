...
#define S_MONAD_(f,v,t) K v; K f(K x){if(!v){U(v=XDB(t)) kap(&KFIXED,v);cd(v);} K k=newK(0,1); U(k) kK(k)[0]=x; K z=vf_ex(&v,k); DO(k->n,kK(k)[i]=0) cd(k); R z; }
#define S_MONAD(f,x) S_MONAD_(_##f,f##_KVAR,x)
#define S_DYAD_(f,v,t) K v; K f(K x,K y){if(!v){U(v=XDB(t)) kap(&KFIXED,v);cd(v);} K k=newK(0,2); U(k) kK(k)[0]=x; kK(k)[1]=y; K z=vf_ex(&v,k); DO(k->n,kK(k)[i]=0) cd(k); R z; }
#define S_DYAD(f,x) S_DYAD_(_##f,f##_KVAR,x)
#define S_TRIAD_(f,v,t) K v; K f(K x,K y,K w){if(!v){U(v=XDB(t)) kap(&KFIXED,v);cd(v);} K k=newK(0,3); U(k) kK(k)[0]=x; kK(k)[1]=y; kK(k)[2]=w; K z=vf_ex(&v,k); DO(k->n,kK(k)[i]=0) cd(k); R z; }
...
S_DYAD(vs,   "{:[@x;{(1_|{ _ y % x}[x]\\y)!x}[x;y]; {|(-1 _ j)-a*1 _ j:y(_%)\\a:|x}[x;y]]}") //Working subfunction constants would remove [x] projection. Second part from K2 manual
...
K math(F(*f)(F), K a)
{
  ...
  K z=newK(t,n);
  if     (0==    at )DO(n, kK(z)[i]=math(f,kK(a)[i])) //should be no demote needed
