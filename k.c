...
#include "mt.c"//mersenne twister for RF() - 64-bit replacement for rand srand
#include <pthread.h>
#include <arpa/inet.h> 
...
#include "h.h" //Big header file auto-generated by 'makeheaders' program
#include "v.c" //verbs
#include "0.c" //I/O
#include "r.c" //reserved/system
#include "p.c" //parser
#include "c.c" //commands/help
#include "getline.c"
...
{ 
  ...
  if(in(p,vd0))R 2;  
  if(in(p,vm_))R 1; // _abs   (~46 of these)
  if(in(p,vd_))R 2; 
  if(in(p,vt_))R 3;  
  R 0;              // (~100 in total) 
}
...
void showAtDepth(K a, I d, I x, I vdep, I b)
{
  ...
  if(2==ABS(t)) if(!a->n) O("0#0.0"); else DO(a->n, CPMAX g=kF(a)[i];isnan(g)?O("0n"):g==-FI?O("-0i"):g==FI?O("0i"):O("%.*g",(int)PP,g);if(i<_i-1)O(" ");else if(fabs(g-(I)g) < pow(10,-PP))O(".0")) //TODO: 1.0 should not display as 1
  ...
  if(7==    t)
  {
    if(1==a->n)
    {
      ...
      { //TODO: mute extraneous :
        ...
        else if(in(p,vt_)) O("%s",vt_s[p-vt_]);
        else if(in(p,vd_)) O("%s",vd_s[p-vd_]);
        else if(in(p,vm_)) O("%s",vm_s[p-vm_]);
